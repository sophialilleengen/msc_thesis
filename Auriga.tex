...
\\This work uses the Auriga (\citealp{AurigaGrand}, hereafter \citetalias{AurigaGrand}) simulations, which try to recreate spiral galaxies such as our own. 
\begin{figure}[htbp]
...
\citetalias{AurigaGrand} use two different methods two distinguish the components and to get their mass ratio:
\begin{enumerate}
...
\begin{table}[htbp]
\captionsetup{format=plain}
    \caption{Comparison of some of the structural parameters of our results with the Auriga \citepalias{AurigaGrand} analysis and \ac{MW} values taken from \citet{Bland-Hawthorn...MW...2016}.}
    \centering
    \begin{tabular}{@{}lllll@{}}
         \toprule
...
\end{table}
In Table \ref{tab:disk_quant_comparison}, we compare some of our potential quantities to the ones derived in \citetalias{AurigaGrand} and to typical \ac{MW} values. The comparison to the \ac{MW} is not direct but rather to get an idea of typical quantities of spiral galaxies. Also, different methods and models are applied. However, many of our results are in a similar regime, such as scale lengths and the circular velocity. The difference in our work and in \citetalias{AurigaGrand} in masses is due to the decomposition: while \citetalias{AurigaGrand} used the first method described in Section \ref{subsubsec:decomp} to derive these values, we use the second. The difference in scale lengths is due to different models.
